...
### Eigen code!!! ============================================================================
  ...
  for _, value in table:
    for tok in value:
      for char in tok:
        if char in punc and ".m." not in tok:
          tok = tok.replace(char, "") 
      if " " in tok:
        tok = tok.split(" ")
        for individual_token in tok:
          tokens.append(individual_token)
      else:
        tokens.append(tok)
  
  table_values = set(tokens)
...
def parent(predictions,
  ...
  multilingual_stopwords = english_stops.union(dutch_stops)
  ...
  for prediction, list_of_references, table in zip(
    ...
    for reference in list_of_references:
      ...
      for order in range(1, max_order + 1):
        ...
        pred_ngram_weights = {ngram: entailment_fn(ngram, table, stopwords = multilingual_stopwords)
                              for ngram in pred_ngram_counts}
        ref_ngram_counts = _ngram_counts(reference, order)
        ref_ngram_weights = {ngram: entailment_fn(ngram, table, stopwords = multilingual_stopwords)
