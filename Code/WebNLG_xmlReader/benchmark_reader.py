...
class Lexicalisation:
    ...
    def get_text(self, lex_tag):
        for child in lex_tag:
            if child.tag == 'text':
...
def select_files(topdir, category='', size=(1, 8)):
    ...
    # print(f'finaldirs   {finaldirs}')

    finalfiles = []
    # for item in finaldirs:
    #     print(f'item in loop:       {item}')
    #     print(f'finaldirs in loop:       {finaldirs}')

        # for filename in sorted(listdir(item)):
        #     print(f'filename    {filename}')
            # if category in filename and 'xml' in filename:
    ...
    for file in filedir:
        ...
        finalfiles.append((filepath, filename))
